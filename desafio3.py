bagunca = """kAewtloYgcFQaJNhHVGxXDiQmzjfcpYbzxlWrVcqsmUbCunkfxZWDZjUZMiGqhRRiUvGmYmvnJIHEmbT
...
for c in range(len(lista)):
  if c == 4:
    bg1.append(lista[c-4])
    bg1.append(lista[c-3])
    bg1.append(lista[c-2])
    bg1.append(lista[c-1])
    bg2.append(lista[c+1])
    bg2.append(lista[c+2])
    bg2.append(lista[c+3])
    bg2.append(lista[c+4])
    ...
    if lista[c].islower():
      if str(bg1[0]).islower() and str(bg2[-1]).islower():
        if str(bg1[1:]).isupper() and str(bg2[:3]).isupper():
          print(f"{bg1} {lista[c]} {bg2}")
          mensagem.append(lista[c])

str_mensagem = ''.join(mensagem)
print(str_mensagem)